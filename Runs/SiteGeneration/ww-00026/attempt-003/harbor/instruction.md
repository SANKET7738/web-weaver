# Replicate the screenshots and recordings as a React + plain CSS website

You are given full-page screenshots **and video recordings** of every page of a multi-page website. Your job is to replicate each page as faithfully as possible — both the steady-state visual design (from screenshots) and the motion design (from recordings) — using **React** (function components, JSX) styled with **plain CSS**, built into static HTML+CSS+JS with Vite in multi-page mode.
...
Build the site with React (plain CSS) and emit a fully-built static
...
- `/app/site/index.html` for `page_01.png` / `page_01.mp4` (the built
  static HTML produced from your `React (plain CSS)` source for page 1)
...
`@vitejs/plugin-react`. If Vite emits build output to `dist/`, copy or move
