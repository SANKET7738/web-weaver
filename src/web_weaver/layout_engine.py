...
- Aesthetic override permission — use sparingly. If the brand story, target audience, or domain strongly conflicts with the assigned design_aesthetic (e.g., a memorial / hospice / palliative-care brand assigned cyberpunk, a children's pediatric clinic assigned brutalist, a meditation app assigned maximalist_collage), you may dial the aesthetic toward what the story demands rather than executing the conflict literally. Express the override through the palette mood and typographic register — for example, soften cyberpunk neon into a calmer dusk gradient for the memorial brand, or replace brutalist mass with restrained sans-serif for the pediatric clinic. When you deviate meaningfully, say so explicitly inside the most affected color's `usage` note (e.g., "softening the cyberpunk neon to a calmer dusk gradient because the brand is a memorial service" or "replacing brutalist mass with restrained warmth because the target audience is pediatric"). Do not use this permission as an excuse to default toward modern-SaaS — only override when the assigned aesthetic genuinely fights the brand.
- Motion personality (look-ahead, no implementation here): the design system you choose will be paired downstream with motion design — entrance animations, scroll-triggered reveals, hover affordances, ambient looped motion, signature kinetic moments. Saturation, value contrast, and typographic weight all carry motion energy: high-saturation neon palettes hold up under bold kinetic motion; muted heritage palettes read best under subtle, restrained motion; brutalist black-and-white hard-blocks read best under a single bold structural motion gesture. Make sure your palette and typography support the motion personality the aesthetic implies — do not pick a system that fights its own animations. Do not specify any animation code or timing here; the page design plans will handle motion design.
- Stick to the inputs, but err on the side of boldness. Every creative move should still be grounded in the design_aesthetic's traits / visual_cues / typography / palette_mood, the layout_family's structure_cues, or the brand identity — but restraint is the right default *only* for aesthetics that explicitly call for it (heritage, swiss, minimalist, terminal_hacker, corporate_clean). For every other aesthetic, exuberance is the baseline; play it safe and the system reads as generic.
...
- Motion design — this page must have a kinetic dimension alongside its visual one. Specify motion as deliberately as you specify color and type. Cover:
  - The on-load arc: what plays as the page arrives (hero entrance, staggered headline reveal, subhead and CTA follow-in, hero illustration drift, page-load gradient sweep) and roughly when each lands relative to the others (named in human terms — "a beat after the headline settles", not "200ms cubic-bezier(0.4, 0, 0.2, 1)").
  - Scroll-triggered reveals: which sections fade / slide-up / scale in as they enter the viewport, and whether their children stagger. Wire this up downstream with IntersectionObserver in vanilla JS.
  - Hover and focus affordances: how cards, buttons, CTAs, nav items, and interactive images respond to pointer hover and keyboard focus (lift, color shift, shadow grow, scale, underline grow, icon swap).
  - Ambient looped motion: marquee strips, pulsing CTAs, slow gradient sweeps, parallax drifts, blinking cursors, breathing icons — whatever fits the aesthetic.
  - A *signature motion moment* tied to or amplifying the page's signature visual moment. Name it explicitly and place it.
  Calibrate intensity to the brand and aesthetic: playful / consumer / friendly_consumer_app / maximalist / vaporwave / cyberpunk / retro_90s_web / memphis pages should carry 6–10 motion moments per page and err on the side of *more*, not less — a playful page with only 3 motion moments is a missed opportunity; editorial / heritage / luxury pages should carry 2–3 restrained moments; minimalist / swiss pages should use motion sparingly and only on structural or typographic moves; brutalist pages may carry only one bold structural motion gesture. Every motion should have intent, but for the high-energy aesthetics above the intensity calibration is a floor, not a ceiling.
- Motion implementation constraints (informational — guides downstream): motion will be implemented with whatever web platform primitives or animation libraries fit best — CSS `@keyframes` / `transition` / `transform`, `IntersectionObserver`, `requestAnimationFrame`, or third-party animation libraries (GSAP, Anime.js, Framer Motion, AOS, Lottie, ScrollMagic, motion-one, react-spring, etc.) at the implementer's discretion. Specify motion in human terms — "a quick 200ms hover lift", "a slow ambient pulse every two seconds", "the marquee scrolls left at a steady, hypnotic pace" — rather than CSS code or library API calls.
- Stick to the inputs, but err on the side of boldness. Every creative move should still be grounded in the design_aesthetic's traits / visual_cues / typography / palette_mood, the layout_family's structure_cues, the page's role / goal, the section's type / intent / items / asset_ideas, or the brand identity — but restraint is the right default *only* for aesthetics that explicitly call for it (heritage, swiss, minimalist, terminal_hacker, corporate_clean). For every other aesthetic, exuberance is the baseline.
...
- page_level_design_instruction must describe the full page in detail: section sequence and how the rhythm varies, color and surface usage across sections, typographic hierarchy and its expressive moves, density / whitespace logic, the page's signature moment (named explicitly), the page's motion design (on-load arc, scroll-triggered reveals, hover affordances, ambient looped motion, and the named signature motion moment — all in human terms, no CSS), responsive behavior, and how this page fits the shared design system while still being its own composition.
- Each section nl_prompt must be detailed and specific to that section. Use its type, intent, eyebrow, headline, subheadline, body, CTAs, items, and asset ideas. Spell out exact alignment, grid columns, typographic scale and weight, color tokens used, padding / spacing rhythm, and any section-level signature move where one exists. When motion is part of how the section reads — entrance, scroll reveal, looped ambient, hover, special interaction — describe it briefly in human terms ("the four stat numbers scroll left in a slow continuous marquee", "the card lifts 4px and the shadow deepens on hover"). If a section is deliberately still, say so.
